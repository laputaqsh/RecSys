...
public class Html2Json {
    ...
    private static List<Map<String, Object>> html2json(List<Map<String, Object>> nodes, List<Node> nodeList) {
        for (Node node : nodeList) {
            String nodeName = Html2Json.replaceNodeName(node.nodeName());
            if (!Html2Json.nodeFilter(nodeName)) {
                continue;
            }
            ...
            for (Attribute attribute : attributes) {
                if (Html2Json.attrFilter(attribute.getKey())) {
            ...
            Html2Json.html2json(childrens, node.childNodes());
